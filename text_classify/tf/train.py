...
MODEL_FILE = DATA_PATH + 'model/model.tflearn'
WORDS_FILE = DATA_PATH + 'model/words_data.pkl'
...
def parseData():
    ...
    for intent in train_data:
        ...
        for sentence in intent['sentences']:
            ...
            bow += [w for w in words if w not in bow]
            # make a doc
            docs.extend( [(words, intent['intent'])] )
...
def prepareDataForTf(classes, bow, docs):
    ...
    for doc in docs:
        ...
        for word in bow:
            bag = bag+[1] if word in doc[0] else bag+[0]
